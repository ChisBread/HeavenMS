...
function snowmanHeal(eim) {
  ...
  var snowman = eim
    .getInstanceMap(entryMap)
    .getMonsterById(9400316 + 5 * difficulty + 5);
...
function monsterKilled(mob, eim) {
  try {
    ...
    var itemObj = new Item(
      forceDrop || Math.random() < 0.77 ? 4032094 : 4032095,
      0,
      1
...
function snowmanEvolve(eim, curLevel) {
  var mapobj = eim.getInstanceMap(entryMap);
  var difficulty = eim.getIntProperty("level");
  var snowman = mapobj.getMonsterById(
    9400317 + 5 * difficulty + (curLevel - 1)
  );
  ...
  var snowman = MapleLifeFactory.getMonster(
    9400317 + 5 * difficulty + curLevel
  );
...
function snowmanSnack(eim) {
  ...
  if (step >= 2 + eim.getIntProperty("level") * snowmanLevel) {
    ...
    var difficulty = eim.getIntProperty("level");
    var snowman = mapobj.getMonsterById(
      9400316 + 5 * difficulty + snowmanLevel
    );
