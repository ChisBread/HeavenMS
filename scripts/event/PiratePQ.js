...
function respawnStages(eim) {
  ...
  if (stg < 3) {
    // thanks Chloek3, seth1, BHB for suggesting map respawn rather than waves on stg2
    eim
      .getMapInstance(925100100)
      .spawnAllMonsterIdFromMapSpawnList(
        9300114 + stg,
        eim.getIntProperty("level"),
        true
      );
  }
...
function isLordPirate(mob) {
  var mobid = mob.getId();
  return (
    mobid == 9300105 || mobid == 9300106 || mobid == 9300107 || mobid == 9300119
  );
}
...
function monsterKilled(mob, eim) {
  ...
  if (map.countMonsters() == 0) {
    ...
    if ((stage == 1 || stage == 3 || stage == 4) && passedGrindMode(map, eim)) {
      ...
      if (
        map.getReactorByName("sMob1").getState() >= 1 &&
