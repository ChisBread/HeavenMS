...
function afterSetup(eim) {
  eim.dropMessage(
    5,
    "The first wave will start within 15 seconds, prepare yourselves."
  );
...
function setup(channel) {
  ...
  mob.disableDrops();
  eim
    .getInstanceMap(270050100)
    .spawnMonsterOnGroundBelow(mob, new java.awt.Point(0, -42));
...
function playerEntry(eim, player) {
  eim.dropMessage(
    5,
    "[Expedition] " + player.getName() + " has entered the map."
  );
...
function changedMap(eim, player, mapid) {
  if (mapid < minMapId || mapid > maxMapId) {
    if (eim.isExpeditionTeamLackingNow(true, minPlayers, player)) {
      ...
      eim.dropMessage(
        5,
        "[Expedition] " + player.getName() + " has left the expedition."
      );
      eim.unregisterPlayer(player);
    }
  }
...
function playerDead(eim, player) {
  ...
  if (count == 10) {
    eim.dropMessage(
      5,
      "[Expedition] Too many players have fallen, Pink Bean is now deemed undefeatable; the expedition is over."
    );
    end(eim);
  } else if (count == 8) {
    eim.dropMessage(
      5,
      "[Expedition] Pink Bean is growing stronger than ever, last stand mode everyone!"
    );
  } else if (count == 6) {
    eim.dropMessage(
      5,
      "[Expedition] Casualty count is starting to get out of control. Battle with care."
    );
  }
}
  ...
  if (isPinkBean(mob)) {
    ...
    if (noJrBossesLeft(mob.getMap())) {
      ...
      if (stage == 5) {
        ...
        mapObj.spawnItemDrop(
          dropper,
          dropper,
          itemObj,
          reactObj.getPosition(),
          true,
          true
        );
        ...
        eim.dropMessage(
          6,
          "With the last of its guardians fallen, Pink Bean loses its invulnerability. The real fight starts now!"
        );
      } else {
        stage++;
        ...
        eim.dropMessage(
          5,
          "The next wave will start within 15 seconds, prepare yourselves."
        );
        eim.schedule("startWave", 15 * 1000);
      }
    }
